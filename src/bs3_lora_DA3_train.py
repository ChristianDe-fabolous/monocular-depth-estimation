...
TEACHER_MODEL      = os.getenv("TEACHER_MODEL", "DA3NESTED-GIANT-LARGE-1.1")
...
WARMUP_STEPS       = 2000
VAL_SPLIT          = 0.1
VAL_INTERVAL_STEPS = int(os.getenv("VAL_INTERVAL_STEPS", 0))
NUM_WORKERS        = 0
AMP                = True
SEED               = 42
LOG_INTERVAL       = 20
...
WANDB_RUN_NAME = f"baseline3-lora-r16-{STUDENT_MODEL}"
...
class PseudoLabelDataset(Dataset):
    ...
    def _depth_path(self, img_path: Path) -> Path:
        return self.cache_dir / img_path.name.replace("_rgb.png", "_depth.npy")
...
def load_model(device: torch.device) -> DepthAnything3:
    ...
    print(f"Trainable: {n_train:,} / {n_total:,} params  (LoRA r={LORA_R} alpha={LORA_ALPHA})")
...
def train_one_epoch(model, loader, optimizer, scaler, scheduler, device, epoch: int, val_loader=None) -> float:
    model.train()
    total = 0.0
    for i, (images, depths) in enumerate(loader):
        ...
        if VAL_INTERVAL_STEPS > 0 and val_loader is not None and (i + 1) % VAL_INTERVAL_STEPS == 0:
            global_step = (epoch - 1) * len(loader) + i
            val_metric = validate(model, val_loader, device)
            print(f"  [val @ step {i+1}] si_rmse={val_metric:.4f}")
            wandb.log({"val_si_rmse_step": val_metric}, step=global_step)
            model.train()
...
def generate_pseudo_labels(train_dir: Path, cache_dir: Path, device: str) -> bool:
    ...
    for p in image_paths:
        prediction = model.inference(image=[str(p)], process_res=560, process_res_method="upper_bound_resize")
        depth = prediction.depth[0]
        assert depth.shape == (560, 560)
        ...
        np.save(cache_dir / p.name.replace("_rgb.png", "_depth.npy"), cache_depth)
        n_written += 1
...
def main():
    ...
    if not os.path.exists(args.cache) and not args.pseudo_label:
        raise FileNotFoundError(f"Cache {args.cache} does not exist. Use --pseudo-label to generate.")
    ...
    if cache_dir.exists():
        ...
        if not generate_pseudo_labels(train_dir, cache_dir, device):
            raise RuntimeError("Failed to generate all pseudo-labels.")
    ...
    output_dir = Path(SCRATCH_ROOT, f"models/{WANDB_RUN_NAME}-{current_datetime}")
    ...
    val_manifest = os.getenv("VAL_MANIFEST")
    wandb.init(
        project=WANDB_PROJECT,
        name=f"{WANDB_RUN_NAME}-{current_datetime}",
        config={
    ...
    if val_manifest and Path(val_manifest).exists():
        print(f"Using val manifest: {val_manifest}")
        train_idx, val_idx = load_val_manifest(Path(val_manifest), full_ds)
        if len(val_idx) == 0:
            print("Warning: manifest matched 0 images, falling back to random split")
            val_manifest = None
    if not val_manifest or not Path(val_manifest or "").exists():
        n   = len(full_ds)
    ...
    for epoch in range(1, EPOCHS + 1):
        print(f"\nEpoch {epoch}/{EPOCHS}")
        train_loss = train_one_epoch(model, train_loader, optimizer, scaler, scheduler, device, epoch, val_loader)
        val_metric = validate(model, val_loader, device)
    ...
    for i in range(0, len(image_paths), INFER_BATCH):
        ...
        predictions = model.inference(image=[str(p) for p in img_batch], process_res=560, process_res_method="upper_bound_resize")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Baseline 3 LoRA r=16: fine-tune student backbone on teacher pseudo-labels.")
    parser.add_argument("--cache", type=Path, required=True, help="Folder containing pseudo-labeled training data.")
    parser.add_argument("--pseudo-label", action="store_true", help="Generate pseudo-labels before training.")
